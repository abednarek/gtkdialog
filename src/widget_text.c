...
void widget_text_refresh(variable *var)
{
	...
	if (var->widget_tag_attr &&
		(value = get_tag_attribute(var->widget_tag_attr, "scrollable")))
		scrollable = widget_attribute_is_true(value);

	if (!initialised) {
		if (scrollable && var->widget_tag_attr &&
			(value = get_tag_attribute(var->widget_tag_attr,
				"scroll-to-bottom")) && widget_attribute_is_true(value))
...
static void widget_text_input_by_command(variable *var, char *command)
{
	...
	if (infile = widget_opencommand(command)) {
		...
		if (gtk_label_get_use_markup(GTK_LABEL(var->Widget))) {
			...
			gtk_label_set_text(GTK_LABEL(var->Widget), text);
		}
	g_free(text);
