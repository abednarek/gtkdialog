...
static void widget_list_item_set_value(GtkWidget *item, const gchar *value);
...
static void widget_list_item_set_value(GtkWidget *item, const gchar *value)
{
	gchar *saved = g_strdup(value);

	g_object_set_data_full(G_OBJECT(item), "_gtkdialog-list-value",
		saved, g_free);
	gtk_object_set_user_data(GTK_OBJECT(item), saved);
}
...
gchar *widget_list_envvar_construct(GtkWidget *widget)
{
	...
	if (item != NULL) {
		string = g_strdup(gtk_object_get_user_data(item->data));
	} else {
		string = g_strdup("");
	}
	...
	gint command_or_file)
{
	...
	if (infile) {
		while ((line = widget_read_line(infile)) != NULL) {
			...
			widget_list_item_set_value(item, line);
...
static void widget_list_input_by_items(variable *var)
{
	...
	while (text != NULL) {
		 ...
		 * I've added it */
		gtk_widget_show(item);
		widget_list_item_set_value(item, text);
		gtk_container_add(GTK_CONTAINER(var->Widget), item);
