		...
		if (strncasecmp(act, "file:", 5) == 0) {
			if ((stock_name = attributeset_get_this_tagattr(&element,
				...
				if (attr &&
					...
					stock_icon_size = widget_parse_stock_icon_size(value,
						GTK_ICON_SIZE_DND, "pixmap stock-icon-size");
			...
			if (strlen(act) > 5) {
				...
				if (width == -1 && height == -1) {
					...
					widget = gtk_image_new_from_file(resolved_file_name);
				} else {
...
static void widget_pixmap_input_by_file(variable *var, char *filename)
{
	...
	if (attributeset_is_avail(var->Attributes, ATTR_WIDTH))
		width = widget_parse_size(attributeset_get_first(&element,
			var->Attributes, ATTR_WIDTH), "pixmap width");
	if (attributeset_is_avail(var->Attributes, ATTR_HEIGHT))
		height = widget_parse_size(attributeset_get_first(&element,
			var->Attributes, ATTR_HEIGHT), "pixmap height");

	resolved_filename = find_pixmap(filename);
	if (width == -1 && height == -1) {
		/* Handle unscaled images */
		gtk_image_set_from_file(GTK_IMAGE(var->Widget), resolved_filename);
	} else {
		/* Handle scaled images */
		pixbuf = gdk_pixbuf_new_from_file_at_size(
